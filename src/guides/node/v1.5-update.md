...
To help clarify the detauls, the Smoothing Pool uses the following rules:
...
After three strikes, the minipool will begin receiving **infractions** - each infraction will dock **10% of its Beacon Chain ETH earnings** and send them to the rETH pool stakers upon withdrawing funds from the minipool.
...
The faucet run in our Discord bot by community member **Patches** has been adapted to support Ropsten.
